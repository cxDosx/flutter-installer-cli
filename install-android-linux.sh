...
#   or locally:
#   bash install-android-linux.sh [OPTIONS]
#
# Options:
...
usage() {
    cat <<'EOF'
Usage: bash install-android-linux.sh [OPTIONS]
...
Examples:
  ...
  bash install-android-linux.sh --non-interactive
  bash install-android-linux.sh -y --sdk 34 --ndk 27.0.12077973
  bash install-android-linux.sh -y --flutter 3.24.0
EOF
}
